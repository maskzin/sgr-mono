...
export class EmployeeService {
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    return Object.assign({}, employee, {
      createdAt: employee.createdAt?.isValid() ? employee.createdAt.format(DATE_FORMAT) : undefined,
      updateAt: employee.updateAt?.isValid() ? employee.updateAt.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        ...
        employee.updateAt = employee.updateAt ? dayjs(employee.updateAt) : undefined;
      });
    }
    return res;
  }
